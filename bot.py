...
def xac_dinh_dung_than(ls):
    ...
    if diem_tuong_tro <= 2.0:
        ...
        # Lọc None để tránh crash khi join display
        return [h for h in [hanh_manh_nhat, hanh_sinh_manh] if h is not None], "Tòng Nhược (Đặc Biệt)"
...
def phan_tich_chuyen_gia_3_mon(ngay_check: date, ls: dict):
    ...
    # Trường Sinh nhân vào trade/study/move/work (chưa tính TS state trực tiếp)
    # s_health KHÔNG nhân he_so_ts vì đã tích hợp TS state trực tiếp vào công thức (+3/-3)
