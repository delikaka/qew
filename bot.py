...
# ============================================================
# TOÁN HỌC TỨ TRỤ & HỆ SỐ NĂNG LƯỢNG CAO CẤP
# ============================================================
...
# ------------------------------------------------------------
# LOGIC CHUYÊN GIA NÂNG CAO [Bổ sung mới]
# ------------------------------------------------------------
def tinh_suc_manh_nhat_chu(ls):
    """Xác định hệ số sức mạnh Nhật Chủ (Cá nhân hóa theo bối cảnh)"""
    nh_chu = NGU_HANH[ls["nhat_chu"]]
    chi_thang = ls["thang"]["chi"]
    mua_vuong = {
        "Mộc": ["Dần", "Mão", "Hợi"], "Hỏa": ["Tỵ", "Ngọ", "Dần"],
        "Thổ": ["Sửu", "Thìn", "Mùi", "Tuất"], "Kim": ["Thân", "Dậu", "Sửu"],
        "Thủy": ["Tý", "Hợi", "Dậu"]
    }
    return 1.2 if chi_thang in mua_vuong.get(nh_chu, []) else 0.8

def get_dich_ma(chi):
    """Tính toán vector di chuyển (Dịch Mã)"""
...
def phan_tich_chuyen_gia_3_mon(ngay_check: date, ls: dict):
    """Phân tích 3 hạng mục: Trading, Học tập, Di chuyển [Cải tiến từ cite: 2]"""
    ...
    # 📈 TRADING: Hệ số cân bằng giữa Tài và Thân
    s_trade = 5.0
    if thap_than in ["Thiên Tài", "Chính Tài"]:
        s_trade += (3.5 * suc_manh) # Thân vượng mới gánh được Tài
    elif thap_than == "Kiếp Tài":
        s_trade -= 3.0
    if frozenset({chi_ngay, ls["ngay"]["chi"]}) in LUC_XUNG: s_trade -= 2.5

    # 📚 HỌC TẬP: Hệ số hấp thụ kiến thức (Ấn tinh)
    s_study = 5.0
    if thap_than in ["Chính Ấn", "Thiên Ấn"]: s_study += 3.0
    if NGU_HANH[chi_ngay] in ["Thủy", "Mộc"]: s_study += 1.0 # Môi trường tương sinh cho tư duy

    # 🚗 DI CHUYỂN: Vector động lực và Ma sát (Xung khắc)
    s_move = 5.0
    ma_list = [get_dich_ma(ls["nam"]["chi"]), get_dich_ma(ls["ngay"]["chi"])]
    if chi_ngay in ma_list: s_move += 4.5
    if frozenset({chi_ngay, ls["ngay"]["chi"]}) in LUC_XUNG: s_move -= 4.0
    
    return {
        "trading": round(max(0, min(10, s_trade)), 1),
        "study": round(max(0, min(10, s_study)), 1),
        "move": round(max(0, min(10, s_move)), 1)
    }

def phan_tich_ngay_sau(ngay_check: date, gio: int, sinh_info: dict):
    """Tính điểm Hung/Kỵ cơ bản[cite: 1, 2]"""
    ls = sinh_info["la_so"]
    _, month_chi = get_tiet_khi(ngay_check)
    ...
    for n_now, c_now, p_coeff in check_list:
        for n_tar, c_tar, weight in targets:
            if frozenset({c_now, c_tar}) in LUC_XUNG:
                current_score = weight * p_coeff * (1.3 if NGU_HANH.get(c_now) == NGU_HANH.get(month_chi) else 1.0)
    ...
    if tinh_thap_than(ls["nhat_chu"], tt_now["ngay"]["can"]) == "Thất Sát":
        diem_hung += 5; chi_tiet.append(f"⚔️ Thiên Can phạm Thất Sát")

    muc = "🔴 CỰC NẶNG" if diem_hung >= 12 else "🟠 RẤT NẶNG" if diem_hung >= 7 else "🟡 TRUNG BÌNH" if diem_hung >= 3 else "✅ BÌNH THƯỜNG"
    return {"diem": round(diem_hung, 1), "muc": muc, "detail": chi_tiet, "is_dangerous": diem_hung >= 7}

# ============================================================
# DB & BOT HANDLERS [Giữ nguyên cấu trúc cite: 1, 2]
# ============================================================
DB_PATH = "daiky.db"
def init_db():
...
async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    txt = "🌟 *BOT ĐẠI KỴ - PHIÊN BẢN CHUYÊN GIA*\n\n📜 *MENU:* /nhapngaysinh, /canhbao, /ngaydaiky, /homnay"
    await u.message.reply_text(txt, parse_mode="Markdown")

async def nhap_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text("Nhập NĂM SINH (vd: 2009):"); return NHAP_N
...
async def nhap_g(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    ngay_sinh = date(n, t, d); _, tc = get_tiet_khi(ngay_sinh)
    ...
    await u.message.reply_text("✅ Đã cập nhật lá số chuyên gia!"); return ConversationHandler.END
...
async def cmd_hom_nay(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    exp = phan_tich_chuyen_gia_3_mon(date.today(), info["la_so"])
    
    def bar(s): return "🟢" * int(s/2) + "⚪" * (5 - int(s/2))
    txt = f"☀️ *KHÍ VẬN CHI TIẾT:*\n━━━━━━━━━━\n"
    txt += f"💰 Trading: {exp['trading']} {bar(exp['trading'])}\n"
    txt += f"📚 Học tập: {exp['study']} {bar(exp['study'])}\n"
    txt += f"🚗 Di chuyển: {exp['move']} {bar(exp['move'])}\n\n"
    txt += f"*Tổng quan:* {res['muc']} ({res['diem']}đ)\n" + "\n".join(res['detail'])
    await u.message.reply_text(txt, parse_mode="Markdown")

async def cmd_canh_bao(u: Update, c: ContextTypes.DEFAULT_TYPE):
    info = get_data(u.effective_user.id); today = date.today(); warns = []
    if not info: return
    for i in range(1, 31):
        d = today + timedelta(days=i); r = phan_tich_ngay_sau(d, 12, info)
        if r["is_dangerous"]: warns.append(f"📅 *{d.strftime('%d/%m')}* ({r['diem']}đ): {r['muc']}")
    await u.message.reply_text("⚠️ *QUÉT 30 NGÀY TỚI*\n\n" + ("\n".join(warns) if warns else "✅ Bình an."), parse_mode="Markdown")
...
async def cmd_ngay_dai_ky(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    curr = date(2026, m, 1); msg = [f"📅 *NGÀY XUNG THÁNG {m}/2026*"]
    while curr.month == m:
        r = phan_tich_ngay_sau(curr, 12, info)
        if r["is_dangerous"]: msg.append(f"• {curr.strftime('%d/%m')}: {r['muc']} ({r['diem']}đ)")
        curr += timedelta(days=1)
    await u.message.reply_text("\n".join(msg), parse_mode="Markdown")

def main():
    init_db(); app = Application.builder().token(BOT_TOKEN).build()
    conv = ConversationHandler(entry_points=[CommandHandler("nhapngaysinh", nhap_start)], states={NHAP_N:[MessageHandler(filters.TEXT & ~filters.COMMAND, nhap_n)], NHAP_T:[MessageHandler(filters.TEXT & ~filters.COMMAND, nhap_t)], NHAP_D:[MessageHandler(filters.TEXT & ~filters.COMMAND, nhap_d)], NHAP_G:[MessageHandler(filters.TEXT & ~filters.COMMAND, nhap_g)]}, fallbacks=[])
    app.add_handler(conv); app.add_handler(CommandHandler("start", cmd_start)); app.add_handler(CommandHandler("canhbao", cmd_canh_bao)); app.add_handler(CommandHandler("ngaydaiky", cmd_ngay_dai_ky)); app.add_handler(CommandHandler("homnay", cmd_hom_nay))
    app.run_polling()
